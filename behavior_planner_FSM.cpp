...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // define maximum acceleration when stopping
  double a_max = 2;// m/sec (0.2g)
    
  // do not look ahead more than 100 meters.   
  auto look_ahead_distance = min(100.0, velocity_mag*velocity_mag/(2.0*a_max));  // sqrt(v^2/(2*a_max)
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      //_stop_line_buffer is = -1 so 180 turn is already built in??
